...
class Command(BaseCommand):
    help = "Evaluate Uyghur morphology against the Uyghur benchmark"
    ...
    def handle(self, *args, **options):
        ...
        bench_file = os.path.join(bench_dir, "uyghur_morphology_benchmark.json")
        reports_dir = os.path.join("backend", "data", "reports")
        os.makedirs(reports_dir, exist_ok=True)
        stats_path = os.path.join(reports_dir, "uyghur_morphology_statistics.json")
        report_path = options.get("report") or "UYGHUR_MORPHOLOGY_REPORT.md"
        ...
        for case in benchmark:
            ...
            analyses = analyze(surface, "ug", max_results=50)
            if analyses:
                coverage += 1
        ...
        rules_path = os.path.join("backend", "data", "morphology", "uyghur_rules.json")
        lemmas_path = os.path.join("backend", "data", "morphology", "uyghur_lemmas.json")
        rule_count = 0
        lemma_count = 0
        ...
        with open(report_path, "w", encoding="utf-8") as md:
            ...
            md.write("- Data files include `historical_lineage` metadata for Proto Turkic -> Old Turkic -> Uyghur -> Uzbek linking.\n")
            md.write("- Lemma entries include a `latin_transliteration` field for future transliteration alignment.\n")
            md.write("- Rules are file-backed and language-scoped under `ug` for reuse by Historical Forms services.\n")
