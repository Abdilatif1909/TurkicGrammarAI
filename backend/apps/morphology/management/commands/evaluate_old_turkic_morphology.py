...
class Command(BaseCommand):
    help = "Evaluate Old Turkic historical morphology against the Old Turkic benchmark"
    ...
    def handle(self, *args, **options):
        ...
        bench_file = os.path.join(bench_dir, "old_turkic_morphology_benchmark.json")
        reports_dir = os.path.join("backend", "data", "reports")
        os.makedirs(reports_dir, exist_ok=True)
        stats_path = os.path.join(reports_dir, "old_turkic_morphology_statistics.json")
        report_path = options.get("report") or "OLD_TURKIC_MORPHOLOGY_REPORT.md"
        ...
        for case in benchmark:
            ...
            analyses = analyze(surface, "otk", max_results=50)
            if analyses:
                coverage += 1
            ...
            if rank == 1:
                ...
                if len(failures) < 100:
                    failures.append({
                        ...
                        "latin_transliteration": case.get("latin_transliteration"),
        ...
        rules_path = os.path.join("backend", "data", "morphology", "old_turkic_rules.json")
        lemmas_path = os.path.join("backend", "data", "morphology", "old_turkic_lemmas.json")
        rule_count = 0
        lemma_count = 0
        ...
        with open(report_path, "w", encoding="utf-8") as md:
            md.write("# Old Turkic Morphology Report\n\n")
            md.write("## Inventory\n\n")
            md.write(f"- Rules: {rule_count}\n")
            md.write(f"- Lemmas: {lemma_count}\n")
            md.write(f"- Benchmark cases: {total}\n")
            md.write("- Script: Old Turkic runiform\n")
            md.write("- Optional transliteration: Latin metadata fields included\n")
            md.write("- Historical lineage: Proto Turkic -> Old Turkic -> Uyghur -> Uzbek; Proto Turkic -> Old Turkic -> Turkish\n\n")
            md.write("## Evaluation\n\n")
            ...
            md.write("- Rules and lemmas include `historical_lineage` metadata for Proto Turkic -> Old Turkic -> Uyghur -> Uzbek and Proto Turkic -> Old Turkic -> Turkish paths.\n")
            md.write("- Lemma entries include `latin_transliteration` for alignment with Latin scholarly forms such as `teŋri`, `kiši`, and `til`.\n")
            md.write("- Lemma entries include `cognate_set` keys for Cognates Engine integration.\n")
