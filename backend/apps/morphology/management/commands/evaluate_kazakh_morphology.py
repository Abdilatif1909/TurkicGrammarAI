...
class Command(BaseCommand):
    help = "Evaluate Kazakh morphology against the Kazakh benchmark"
    ...
    def handle(self, *args, **options):
        ...
        bench_file = os.path.join(bench_dir, "kazakh_morphology_benchmark.json")
        reports_dir = os.path.join("backend", "data", "reports")
        os.makedirs(reports_dir, exist_ok=True)
        stats_path = os.path.join(reports_dir, "kazakh_morphology_statistics.json")
        report_path = options.get("report") or "KAZAKH_MORPHOLOGY_REPORT.md"
        ...
        for case in benchmark:
            ...
            analyses = analyze(surface, "kk", max_results=50)
            if analyses:
                coverage += 1
        ...
        rules_path = os.path.join("backend", "data", "morphology", "kazakh_rules.json")
        lemmas_path = os.path.join("backend", "data", "morphology", "kazakh_lemmas.json")
        rule_count = 0
        lemma_count = 0
        ...
        with open(report_path, "w", encoding="utf-8") as md:
            md.write("# Kazakh Morphology Report\n\n")
            md.write("## Inventory\n\n")
            ...
            md.write("\n## Vowel Harmony\n\n")
            md.write("- Kazakh Cyrillic major vowel harmony is enforced for productive suffixes.\n")
            md.write("- Examples: `кітаптар`, `үйлер`, and `үйлерімізден` are valid analyses.\n")
