...
class Command(BaseCommand):
    help = "Evaluate Kyrgyz morphology against the Kyrgyz benchmark"
    ...
    def handle(self, *args, **options):
        ...
        bench_file = os.path.join(bench_dir, "kyrgyz_morphology_benchmark.json")
        reports_dir = os.path.join("backend", "data", "reports")
        os.makedirs(reports_dir, exist_ok=True)
        stats_path = os.path.join(reports_dir, "kyrgyz_morphology_statistics.json")
        report_path = options.get("report") or "KYRGYZ_MORPHOLOGY_REPORT.md"
        ...
        for case in benchmark:
            ...
            analyses = analyze(surface, "ky", max_results=50)
            if analyses:
                coverage += 1
        ...
        rules_path = os.path.join("backend", "data", "morphology", "kyrgyz_rules.json")
        lemmas_path = os.path.join("backend", "data", "morphology", "kyrgyz_lemmas.json")
        rule_count = 0
        lemma_count = 0
        ...
        with open(report_path, "w", encoding="utf-8") as md:
            ...
            md.write("\n## Vowel Harmony\n\n")
            md.write("- Kyrgyz Cyrillic four-way vowel harmony is enforced for productive suffixes.\n")
            md.write("- Examples: `китептерибизден`, `үйлөрүбүздөн`, and `адамдар` are valid analyses.\n")
            md.write("- Invalid direct analysis such as `үй + лар` is rejected for `үйлар`.\n")
