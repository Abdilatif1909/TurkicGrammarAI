...
class Command(BaseCommand):
    help = "Evaluate Turkish morphology against the Turkish benchmark"
    ...
    def handle(self, *args, **options):
        ...
        bench_file = os.path.join(bench_dir, "turkish_morphology_benchmark.json")
        reports_dir = os.path.join("backend", "data", "reports")
        os.makedirs(reports_dir, exist_ok=True)
        stats_path = os.path.join(reports_dir, "turkish_morphology_statistics.json")
        report_path = options.get("report") or "TURKISH_MORPHOLOGY_REPORT.md"
        ...
        for case in benchmark:
            ...
            analyses = analyze(surface, "tr", max_results=50)
            if analyses:
                coverage += 1
        ...
        rules_path = os.path.join("backend", "data", "morphology", "turkish_rules.json")
        lemmas_path = os.path.join("backend", "data", "morphology", "turkish_lemmas.json")
        rule_count = 0
        lemma_count = 0
        ...
        with open(report_path, "w", encoding="utf-8") as md:
            md.write("# Turkish Morphology Report\n\n")
            md.write("## Inventory\n\n")
            ...
            md.write("\n## Vowel Harmony\n\n")
            md.write("- Major vowel harmony is enforced for suffixes such as `lar/ler`, `da/de`, and `dan/den`.\n")
            md.write("- Minor vowel harmony is enforced for four-way suffixes such as possessive and accusative variants.\n")
            md.write("- Example: `evler` and `kitaplar` are valid; `evlar` is rejected as a direct `ev + lar` analysis.\n")
